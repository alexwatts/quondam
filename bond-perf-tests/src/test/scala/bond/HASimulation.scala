...
class HASimulation extends Simulation {
  ...
  val bond = conf.getString("urls.bondUrl")
  ...
  val scn = scenario("Claim Keys")
    .during(duration) {
      feed(feeder).exec(http("delete_key")
        .delete(bond + "/keys").body(StringBody("""{ "shard": "${shard}", "key":"${key}" }""")).asJson
  ...
  val chaosScn = scenario("Chaos").
    during(duration) {
      exec(session => {
        val scriptOutput = Process("./bond-perf-tests/chaos.sh").!!
  ...
  setUp(
    ...
    forAll.failedRequests.percent.lt(1) // for each request, < 5% failure
  )
